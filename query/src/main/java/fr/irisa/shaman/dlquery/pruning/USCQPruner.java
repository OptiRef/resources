package fr.irisa.shaman.dlquery.pruning;
...
import fr.irisa.shaman.dlquery.Connector;
import fr.irisa.shaman.dlquery.DL2SQL;
import fr.irisa.shaman.dlquery.QueryUtils;
		...
		for(String v: with.replace("WITH ", "").split("\\n--##\\n")) {
			if(v.startsWith("disj")) {
				...
				if(prunedView.isEmpty()) {
					//System.out.println("We have empty views: "+currViewName);
					...
					if(!prunedView.get(0).contains(viewSelectVar) && !viewSelectVar.contains(",")) {
						//System.out.println("processing view: "+ currViewName);
						//System.out.println("view select : "+ viewSelectVar);
						prunedView.set(0, prunedView.get(0).replace("FROM", " AS "+viewSelectVar+" FROM "));
						/*replaceVar = prunedView.get(0).split("SELECT ")[1].split(" FROM ")[0].split("\\.")[1];
						System.out.println("we replace ##"+ currViewName+"."+viewSelectVar + " with ##"+currViewName+"."+replaceVar);
						select = select.replace(currViewName+"."+viewSelectVar, currViewName+"."+replaceVar);*/
